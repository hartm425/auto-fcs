...
compFrame <- function(frame, file, gt_lymph, d, outputDir, gateDir) {
...
for (files in fcsFilesAll) {
  ...
  for (file in files) {
    ...
    flow_auto_qc(frame,folder_results = "QC",mini_report=paste(basename(file),"mini"),pen_valueFS=200,remove_from = "FR_FM")
    compFrame(
      frame = frame,
      file = file,
      gt_lymph = gt_lymph ,
    ...
    qcFile = paste(tools::file_path_sans_ext(file), "_QC.fcs", sep = "")
    frame = read.FCS(paste(cleanDir,"QC/",qcFile,sep = ""))
    compFrame(
      frame = frame,
      file = file,
      gt_lymph = gt_lymphQC ,
