...
gs <- parseWorkspace(ws, path = "/Users/Kitty/temp/fcs/fcsFiles", name = 1,subset=fcsFiles[1:1],isNcdf = FALSE)
...
gh <- gs[[1]]
plot(gh)
plotGate(gs[[1]],scales=list(cex=c(1.2,1.2)),par.strip.text=list(cex=1.4),xlab=list(fontsize=4),ylab=list(fontsize=12),par.settings=list(gate.text=list(cex=1)),path=2)
...
ggcyto(gs,mapping = aes(x = "CD45RA",y = "CCR7"),subset = "Comp-APC-Cy7-A+Comp-BUV 395-A-") +
 geom_hex(bins=100) + ggcyto_par_set(limits = "data") + geom_gate()+ scale_x_flowJo_biexp()+ scale_y_flowJo_biexp()
template = rbind(
  template,
  add_pop(
    ...
    alias = "CCR7_gate",
    pop = "CCR7",
    parent = "Comp-APC-Cy7-A+Comp-BUV 395-A-/CD45RA+",
    dims =  "CCR7",
    gating_method = "flowClust",
    gating_args = "K=2",
    preprocessing_method = "prior_flowClust",
...
ggcyto(gs,mapping = aes(x = "CD45RA",y = "CCR7"),subset = "Comp-APC-Cy7-A+Comp-BUV 395-A-/CD45RA+") +
 geom_hex(bins=100) + ggcyto_par_set(limits = "data") + geom_gate()
...
template = rbind(
  ...
  add_pop(
    ...
    gating_args = "Comp-APC-Cy7-A+Comp-BUV 395-A-/CD45RA+/CCR7_gate:Comp-APC-Cy7-A+Comp-BUV 395-A-/CD45RA+"
    # preprocessing_method = "prior_flowClust",
    # preprocessing_args = "K=2"
  )
)
