...
fcsFilesAll = unique(c(fcsFilesAllProbs, qcOutliers))
...
compFrame <-
  function(frame,
    ...
    autoCounts =getStats(gs1 = gs1,qcVersion = qcVersion,metric = "count","AUTOMATIC")
    autoProps =getStats(gs1 = gs1,qcVersion = qcVersion,metric = "freq","AUTOMATIC")
    metrics = rbind(autoCounts,autoProps)
    if(!qcVersion){
    g =file  
    gs <- parseWorkspace(ws, path = inputFCSDir, name = 2,subset= g,isNcdf = FALSE,sampNloc="sampleNode")
    manCounts =getStats(gs1 = gs,qcVersion = qcVersion,metric = "count","MANUAL")
    metrics = rbind(metrics,manCounts)

    manProps =getStats(gs1 = gs,qcVersion = qcVersion,metric = "freq","MANUAL")
...
for (files in fcsFilesAll) {
  ...
  for (file in files) {
   ...
   metricBase = compFrame(
      ...
      outputDir = outputDir,
      gateDir = gateDir,
      qcVersion = FALSE,mapper,inputFCSDir
    )
   ...
   metricQC= compFrame(
      ...
      outputDir = outputDir,
      gateDir = gateQCDir,
      qcVersion = TRUE,mapper,inputFCSDir
    )
