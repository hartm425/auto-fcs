...
ggcyto(gs, mapping = aes(x = `FSC-A`, y = `SSC-A`)) + geom_hex(bins = 50) + geom_gate("nonDebris") + xlim(c(0, 2e5))
ggcyto(gs, mapping = aes(x = `FSC-A`, y = `SSC-A`)) + geom_hex(bins = 50) + geom_gate("nonDebris")+ scale_x_flowJo_biexp()+ scale_y_flowJo_biexp()
...
ggcyto(gs,mapping = aes(x = `FSC-A`,y = `SSC-A`)) + geom_hex(bins = 50) + geom_gate("lymph")+ scale_x_flowJo_biexp()+ scale_y_flowJo_biexp()
...
ggcyto(gs,mapping = aes(x = `PE-A`,y = `FSC-H`)) + geom_hex(bins = 50) + geom_gate("PE-A")+ scale_x_flowJo_biexp()+ scale_y_flowJo_biexp()
# 
# # CD3A	CD3+	Live	CD3	mindensity	gate_range=c(1000,2200)
# 
# 
# template = rbind(
#   template,
...
#     gating_method = "mindensity",
#     gating_args =
#       "gate_range=c(1000,2200)"
#   )
# )
# # CD3B	CD3+	CD3A	CD3	mindensity	gate_range=c(1500,2200)
# template = rbind(
#   template,
#   add_pop(
#     gs,
...
#     gating_method = "mindensity",
#     gating_args =
#       "gate_range=c(1500,2200)"
#   )
# )
# # CD3	CD3	Live	CD3	refGate	CD3B
# 
# template = rbind(
#   template,
#   add_pop(
...
#   )
# )
# ggcyto(gs,mapping = aes(x = `CD3`,y = `PE-A`)) + geom_hex(bins = 50) + geom_gate("CD3") + xlim(c(0,2e5))
# ggcyto(gs,mapping = aes(x = `CD3`,y = `PE-A`)) + geom_hex(bins = 50) + geom_gate("PE-A")+ scale_x_flowJo_biexp()+ scale_y_flowJo_biexp()
```
