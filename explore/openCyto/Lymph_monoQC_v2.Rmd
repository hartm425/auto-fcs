...
compP1Frame <-
  function(frame,
    ...
    # gateDef = data.frame(
    #   QC = QCDEF)
    # for(pop in autoCounts$Population){
    #   gateDef = cbind(gateDef,getIndiceMat(gs1,pop))
    # }
    gateDef = data.frame(
      QC = QCDEF,
...
combineWSP <- function(outputDir, gateDir,inputFCSDir) {
  ...
  GatingSet2flowJo(GatingSetList(c(gates1,gates2)),outWsp1 )
...
combineWSP(outputDir =outputDir,gateDir = gateDir,inputFCSDir = inputFCSDir)
