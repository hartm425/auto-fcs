...
gs <- parseWorkspace(ws, path = "/Users/Kitty/temp/fcs/fcsFiles", name = 1,subset=fcsFiles[1:2],isNcdf = FALSE)
...
template = add_pop(
  ...
  gating_args = "min=c(5e4,0),max=c(2.5e5,1e5)"
)
...
template = rbind(
  ...
  add_pop(
    ...
    gating_args = "K=2,quantile=0.90,target=c(1e5,5e4)",
...
ggcyto(gs,mapping = aes(x = `FSC-W`,y = `FSC-H`)) + geom_hex(bins = 50) + geom_gate("sneak")+scale_x_flowJo_biexp()+ scale_y_flowJo_biexp()
# 
# template = rbind(
#   template,
#   add_pop(
