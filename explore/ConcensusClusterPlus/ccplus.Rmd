...
for (type in unique(merge$CELLSID.x)) {
  ...
  for (tp in unique(data$time.point.x)) {
    ...
    colnames(sampleTmp) = c( paste("PBMC",current$KEY2.x[1]),paste("WB",current$KEY2.x[1]))
...
title = "testK10Rep500"
results = ConsensusClusterPlus(as.matrix(sampleBased),maxK=12,reps=1000,pItem=0.8,pFeature=1,title=title,clusterAlg="hc",distance="spearman",seed=1262118388.71279,plot="png",corUse = "pairwise.complete.obs")
