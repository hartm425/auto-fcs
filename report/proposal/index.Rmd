...
OpenCyto gives the user many options to refine algorithmic parameters to improve the performance of each step in the gating hierarchy. We evaluated the performance of our OpenCyto template using internal data for `r sumBase$N_SAMPS` manually gated (Jflow software) samples across `r length(unique(metricCombo$Population.x))` gates. The global correlation between the population counts of manual and OpenCyto gating was high (rho=`r sumAll$RHO` ,p-value <2e-16). Despite a high global concordance, certain subsets were less well correlated (e.g Activated CD4 counts, rho=`r sumActCD$RHO` ,p-value <2e-16).
...
### Phenograph
`r citet("10.1016/j.cell.2015.05.047") `
...
kable(as.data.frame(table(data_xk_all$code,data_xk_all$V1)))

ggplot(data_xk_all , aes(x = tsne_1,y=tsne_2,color=V1)) +geom_point(size = .15)+ guides(colour = guide_legend(override.aes = list(size=4)))
